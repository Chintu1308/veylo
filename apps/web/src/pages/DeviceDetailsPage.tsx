...
export default function DeviceDetailsPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      apiRequest<NetworkEvent[]>(`/projects/${selectedProject.id}/monitoring/events?deviceId=${deviceId}`)
        .catch(() => [] as NetworkEvent[]),
    ])
    ...
    // Real-time network traffic events
  ...
  const isOnline = device.last_seen_at
    ? new Date().getTime() - new Date(device.last_seen_at).getTime() < 3 * 60 * 1000
    : false;
      ...
      <div className="flex flex-col gap-4">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <IconPointFilled 
              size={14} 
              className={socketConnected ? "text-status-low-text animate-pulse" : "text-muted-foreground"} 
            />
        ...
        {events.length === 0 ? (
          <div className="bg-card border border-border border-dashed rounded-xl p-12 flex flex-col items-center justify-center text-center gap-3">
            ...
            <p className="text-xs text-muted-foreground max-w-sm">This device has not generated any network requests yet. Traffic will appear here in real-time once the posture agent starts reporting.</p>
          </div>
        ) : (
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-muted/30 border-b border-border/80">
                  ...
                  <th className="p-4 text-[10px] font-bold text-muted-foreground uppercase tracking-widest">Protocol</th>
                  <th className="p-4 text-[10px] font-bold text-muted-foreground uppercase tracking-widest">Bytes</th>
              ...
              <tbody className="divide-y divide-border/50">
                {events.map((e, idx) => (
                  <tr key={idx} className="hover:bg-muted/10 transition-colors">
                    ...
                    <td className="p-4 text-xs font-mono text-foreground uppercase">{e.protocol || "—"}</td>
                    <td className="p-4 text-xs font-mono text-foreground">{e.bytes_transferred != null ? `${e.bytes_transferred} B` : "—"}</td>
                    <td className="p-4">
